...
|language | Preferred language for the Reply-Message (Two letter ISO language code, e.g. ‘en’).|
...
|terminateCause | Same value as the Acct-Terminate-Cause attribute + `UNKNOWN` value when no information from CoovaChilli has been received yet (-1). Not yet implemented. <ul><li>`User-Request` (1)</li><li>`Lost-Carrier` (2)</li><li>`Idle-Timeout` (4)</li><li>`Session-Timeout` (5)</li><li>`NAS-Reboot` (11)</li><li>`UNKNOWN` (-1)</li></ul>|
...
|redirectionURL | URL to redirect to next, WISPr-Redirection-URL or otherwise|
